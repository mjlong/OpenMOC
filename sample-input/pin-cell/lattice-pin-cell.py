...
if(1==nfr):
  ...
  for ir in range(nfr):
    ...
    if(ir/4):
      ...
      cells[ir].addSurface(halfspace=signs[(isec)*2], surface=vertical)   
      cells[ir].addSurface(halfspace=signs[(isec)*2+1], surface=horizontal)   
  cells[nfr+1].addSurface(halfspace=-1, surface=circle)

#moderator
cells[nfr].addSurface(halfspace=+1, surface=circle)
cells[nfr+2].addSurface(halfspace=+1, surface=circle)
#universe=0
cells[nfr+3].addSurface(halfspace=+1, surface=left)
cells[nfr+3].addSurface(halfspace=-1, surface=right)
cells[nfr+3].addSurface(halfspace=+1, surface=bottom)
cells[nfr+3].addSurface(halfspace=-1, surface=top)
...
plotter.plot_materials(geometry, gridsize=200)
plotter.plot_cells(geometry, gridsize=200)
